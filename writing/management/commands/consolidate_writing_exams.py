...
각 시험(publisher)에는 'X ... 영작 통합' 마스터 단원이 있고, 그 시험의 조각 단원
(외부지문 N, 올림포스 partN, 수능특강 partN 등)은 마스터에 이미 전부 포함된 중복본이다.

이 명령은 publisher 그룹마다:
  1) 제목에 '통합'이 든 마스터를 찾고(여럿이면 문제 수 최다),
  2) 같은 시험의 다른 활성 단원 중 '문장 집합이 마스터의 부분집합'인 조각만 골라
  3) 그 조각의 학생 배정을 마스터로 이전(get_or_create)한 뒤
  4) 조각을 is_active=False 로 비활성화(숨김)한다. 풀이기록(세션)은 보존.
...
'고등 15개씩 / 중등 20개씩, 마지막 1개만 자투리'가 된다.
    ...
    python manage.py consolidate_writing_exams --publisher 동백고2   # 일부만
"""
import re
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--publisher', default='', help='해당 문자열을 포함하는 시험만')

    def handle(self, *args, **opts):
        dry = opts['dry_run']
        pub_filter = (opts['publisher'] or '').strip()

        groups = defaultdict(list)
        for u in WritingUnit.objects.filter(is_active=True):
            groups[u.publisher or ''].append(u)
        ...
        for pub in sorted(groups):
            if pub_filter and pub_filter not in pub:
                continue
            us = groups[pub]
            ...
            fragments = []
            for u in us:
                if u.id == master.id or '통합' in u.title:
                    continue
                usent = _sentence_set(u)
                if usent and usent <= msent:          # 마스터의 부분집합인 중복 조각만
                    fragments.append(u)
